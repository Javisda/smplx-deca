...
def main(args):
    ...
    deca = DECA(config = deca_cfg, device=device) #/// El renderizador está desactivado dentro del modelo
    # identity reference
    i = 0
    ...
    # Access shape params from FLAME
    #shape_betas = id_codedict['shape'] # {Tensor: (1, 100)}
    one_betas = torch.ones(1, 100)
    zero_betas = torch.zeros(1, 100)
    minus_one_betas = - one_betas
    # id_codedict['shape'][:, :10] = one_betas[:, :10] # Overrides the first 10 numbers
    # id_codedict['shape'][:, :] = one_betas[:, :] # Overrides the first 10 numbers
    ...
    for beta in range(0, 1):
        ...
        for j in range(-10, 11):
            if j == -10 or j == 10:
                ...
                x = j / 10.0
                print(str(x))
                tensor = torch.full((1, 100), x, dtype=float)
                id_codedict['shape'][:, :] = torch.zeros(1, 100)
                id_codedict['shape'][:, init_idx:end_idx] = tensor[:, init_idx:end_idx]

                """
                # Cuidado con init_idx:end_idx que habrá que cambiarlo por cómo se calculan con las betas
                if j == -1:
                    id_codedict['shape'][:, init_idx:end_idx] = minus_one_betas[:, init_idx:end_idx]
                elif j == 0:
                    id_codedict['shape'][:, init_idx:end_idx] = zero_betas[:, init_idx:end_idx]
                elif j == 1:
                    id_codedict['shape'][:, init_idx:end_idx] = one_betas[:, init_idx:end_idx]
        
                id_codedict['shape'][:, init_idx:end_idx] = torch.randn(1, 100)
                """
                id_codedict['shape'][:, 0:100] = -torch.randn(1, 100)
                id_opdict = deca.decode(id_codedict, rendering=False, vis_lmk=False, use_detail=False, return_vis=False) #/// El rendering está puesto a True por defecto
                #id_opdict, id_visdict = deca.decode(id_codedict, rendering=False, vis_lmk=False, use_detail=False, return_vis=False) #/// El rendering está puesto a True por defecto
                #id_visdict = {x:id_visdict[x] for x in ['inputs', 'shape_detail_images']}

                # Save id_opdict for testing
                with open("id_opdict.pickle", "wb") as f:
                    import pickle
                    pickle.dump(id_opdict, f)
                ...
                transfer_opdict = deca.decode(id_codedict, rendering=False, vis_lmk=False, use_detail=False, return_vis=False)#/// Añadido
                ...
                for save_type in ['reconstruction']:
                    ...
                    if args.saveObj:
                        save_path = 'TestSamples/manySamples/' + image_name + '' + save_type + '_beta'+str(beta)+ '_(' + str(x) +').obj'
                        deca.save_obj(os.path.join(save_path), opdict)

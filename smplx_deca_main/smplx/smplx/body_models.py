...
from smplx_deca_main.smplx.smplx.lbs import (
    lbs, vertices2landmarks, find_dynamic_lmk_idx_and_bcoords, blend_shapes)

from smplx_deca_main.smplx.smplx.vertex_ids import vertex_ids as VERTEX_IDS
from smplx_deca_main.smplx.smplx.utils import (
...
from smplx_deca_main.smplx.smplx.vertex_joint_selector import VertexJointSelector
from collections import namedtuple

TensorOutput = namedtuple('TensorOutput',
                          ['vertices', 'joints', 'betas', 'expression', 'global_orient', 'body_pose', 'left_hand_pose',
                           'right_hand_pose', 'jaw_pose', 'transl', 'full_pose'])  #/// Solo se ha definido esta estructura para SMPLXLayer. Por qué?
...
class SMPL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if data_struct is None:
            if osp.isdir(model_path):  #/// Devuelve True si model_path es un directorio existente
                model_fn = 'SMPL_{}.{ext}'.format(gender.upper(), ext='pkl')
                #model_fn = 'SMPL_MALE.pkl'
                #model_fn = 'SMPL_FEMALE.pkl'
        ...
        num_betas = 10 # Added para vis_flame_vertices.py

        #/// The code checks if the age attribute of the current instance is equal to 'kid'. If it is, it loads a numpy array from the file specified by kid_template_path, and performs some normalization on it.
        #/// It calculates the difference between the loaded template and the template stored in the data_struct attribute. It then concatenates this difference with the first num_betas of shapedirs and updates shapedirs with the result.
        if self.age == 'kid':
        ...
        shapedirs = shapedirs[:, :, :num_betas] #/// Recorta shapedirs teniendo en cuenta el número de betas correcto, que dependerá (más arriba) por otros factores. Ejemplo shapedirs inicial: [6890, 3, 300] si los betas son 10 por defecto esto se queda en [6890, 3, 10]
        ...
        #/// El método forward no lo entiendo, pero la inicialización es sencilla.
        ...
        if create_betas:
            if betas is None:
                ...
                if torch.is_tensor(betas):
                    default_betas = betas.clone().detach() #/// When a tensor is detached, it no longer requires gradient computation and it can't be used to perform backpropagation. This is useful when you want to stop gradient computation for a specific tensor, either to save memory or to use its value as a constant in future computations.
                else:
                    default_betas = torch.tensor(betas, dtype=dtype)

            #/// Si se hace el detach para las betas que no calculen gradiente más arriba, por qué se registran como Parameter y se habilita el requires_Grad
        ...
        if create_transl:
            if transl is None:
                default_transl = torch.zeros([batch_size, 3],
                                             dtype=dtype,
                                             requires_grad=True) #/// Por qué querríamos registrar operaciones en una traslación por defecto?
            else:
    ...
    #/// The method definition is empty, and there is no implementation. It is likely that this method will be implemented later or in a subclass. The pass statement is used as a placeholder in the code to indicate that this method is intended to do nothing.
    ...
    def reset_params(self, **params_dict) -> None:
        for param_name, param in self.named_parameters():
            if param_name in params_dict:
                param[:] = torch.tensor(params_dict[param_name]) #/// The square bracket notation param[:] is used to modify the entire contents of param.
            else:
                param.fill_(0)
    ...
    def forward(
        ...
        **kwargs
        ...
        batch_size = max(betas.shape[0], global_orient.shape[0], #/// Por qué el batch size se define de esta manera?
                         body_pose.shape[0])
        ...
        #/// Aquí obtenemos los vértices después de aplicar los desplazamientos por pose y forma. Además, se obtienen los nuevos joints desplazados
    ...
    def forward(
        ...
        **kwargs
        ...
        ''' Forward pass for the smpl model
            ...
            -------
        '''
        #/// If inputs are missing, they are filled with default values.
        #/// The forward pass first calculates full_pose by concatenating global_orient and body_pose.
...
class SMPLH(SMPL):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #/// num_pca_comps is a variable that holds the number of principal components to keep. Principal component analysis (PCA) is a statistical technique that is used to identify patterns in data. In this context, it is likely that data_struct is a data structure that holds the results of a PCA analysis on hand-related data. The hands_componentsl and hands_componentsr attributes likely hold the principal components of the PCA analysis, and the slicing operation is used to keep only the first num_pca_comps components.
        ...
        hand_pose_dim = num_pca_comps if use_pca else 3 * self.NUM_HAND_JOINTS #/// Por qué num_pca_comps, que por defecto es 6?
        ...
        # Create the buffer for the mean pose. #/// Mean pose not only for hand but the entire body
    ...
    def forward(
        ...
        **kwargs
        ...
        # ones from the module
        global_orient = (global_orient if global_orient is not None else
                         self.global_orient) #/// self.global_orient en qué momento se define como para poder usarlo? Lo mismo para las siguientes
        body_pose = body_pose if body_pose is not None else self.body_pose
        betas = betas if betas is not None else self.betas
        ...
        joints = self.vertex_joint_selector(vertices, joints) #/// Hay que revisitar esta clase y su funcion
        if self.joint_mapper is not None:
            joints = self.joint_mapper(joints) #/// Investigar tambien el mapper este
    ...
    def forward(
        ...
        **kwargs
        ...
        for var in model_vars:
            if var is None:
                continue
            batch_size = max(batch_size, len(var)) #/// Por qué se hacen estas cosas? No deberían de ser todos del tamaño apropiado de forma inicial?
        device, dtype = self.shapedirs.device, self.shapedirs.dtype
        if global_orient is None:
            global_orient = torch.eye(3, device=device, dtype=dtype).view(
                1, 1, 3, 3).expand(batch_size, -1, -1, -1).contiguous() #/// contiguous(): This returns a contiguous tensor, which means that its memory is stored in a contiguous block of memory. This can be important for some operations, such as converting the tensor to a numpy array.
        if body_pose is None:
            body_pose = torch.eye(3, device=device, dtype=dtype).view(
...
class SMPLX(SMPLH):
    ...
    def __init__(
        ...
        **kwargs
        ...
        lmk_faces_idx = data_struct.lmk_faces_idx #/// The term lmk in the code likely stands for "Landmark". "Landmark" is a term that is commonly used in computer vision and image processing to refer to specific points of interest in an image. In the context of this code, the lmk_faces_idx variable is likely storing the indices of landmarks in a 3D face model.
        self.register_buffer('lmk_faces_idx',
                             torch.tensor(lmk_faces_idx, dtype=torch.long))
        lmk_bary_coords = data_struct.lmk_bary_coords #/// It's likely that lmk_bary_coords is a numpy array or similar data structure, where each row represents the barycentric coordinates of a landmark, and data_struct is a larger data structure or object that contains information about the landmarks.
        ...
        if self.use_face_contour:
            ...
            neck_kin_chain = find_joint_kin_chain(self.NECK_IDX, self.parents) #/// Encuentra el camino de padres (indices) hasta el root del cuerpo
            self.register_buffer(
                'neck_kin_chain',
        ...
        #/// No entiendo muy bien la lógica en el if else siguientes sobre los shapedirs
        shapedirs = data_struct.shapedirs
        if len(shapedirs.shape) < 3:
            shapedirs = shapedirs[:, :, None] #/// The code line shapedirs = shapedirs[:, :, None] appears to be adding an additional dimension to the numpy array shapedirs. The code is using numpy's None keyword to expand the dimensions of the array. It will have shape (m, n, 1)
        if (shapedirs.shape[-1] < self.SHAPE_SPACE_DIM +
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.use_pca: #/// No entiendo muy bien estas operaciones
            left_hand_pose = torch.einsum(
                'bi,ij->bj', [left_hand_pose, self.left_hand_components])
            right_hand_pose = torch.einsum(
                'bi,ij->bj', [right_hand_pose, self.right_hand_components])
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.vertex_joint_selector.extra_joints_idxs = to_tensor(
            list(VERTEX_IDS['smplh'].values()), dtype=torch.long)
...
class FLAME(SMPL):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(FLAME, self).__init__(
            ...
            dtype=dtype,
            batch_size=batch_size,
            vertex_ids=VERTEX_IDS['smplx'], # Added
            gender=gender,
            ext=ext,
            **kwargs)
        ...
        if create_neck_pose:
            ...
            neck_pose_param = nn.Parameter( #/// https://stackoverflow.com/questions/50935345/understanding-torch-nn-parameter
    ...
    def forward(
        ...
        **kwargs
        ...
        scale = int(batch_size / betas.shape[0])
        if scale > 1:
            betas = betas.expand(scale, -1) #/// Simplemente añade más arrays de betas para completar el tamaño de modelos de entrada
        shape_components = torch.cat([betas, expression], dim=-1)
        shapedirs = torch.cat([self.shapedirs, self.expr_dirs], dim=-1)
...
def build_layer(
    ...
    model_type: str = 'smpl',
    **kwargs
) -> Union[SMPLLayer, SMPLHLayer, SMPLXLayer, MANOLayer, FLAMELayer]: #/// The "Union" here is a type hint in Python that specifies that the return value of the "build_layer" function can be either one of several types.
    ...
    elif 'smplh' in model_type.lower():
